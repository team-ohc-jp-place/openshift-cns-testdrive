...
[TIPS]
====
NOTE: `$ oc project` でProject（Namespace）を指定しておくことで、``-n lab1-1`` のようにコマンド実行時に毎度Namespace指定をする必要が無くなるため幾分便利になります。 +
※ただし、本ハンズオンの実行例においては、``$ oc project +++<Project名>+++` の実行有無に関係無く `-n` オプションを付与しています。
====
+
...
[TIPS]
====
CAUTION: コマンド実行時は、`-n <各自のProject名>` に読み替えてください。
====
+
