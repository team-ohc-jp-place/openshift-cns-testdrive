...
In a previous lab, you created "infra" nodes leveraging https://docs.openshift.com/container-platform/4.3/nodes/scheduling/nodes-scheduler-node-selectors.html[nodeSelector]
...
汚染された (Tainted) ノード上でポッドが動作しないようにする方法があります。

Some usecases include:
以下のようなユースケースが考えられるでしょう。：
...
このラボでは、Taints と Tolerations を使用してワークロードを制御する方法を探ります。


### 背景

Taints と Tolerations は、協調して動作し、ワークロードが他のノードにスケジュールされないようにします。
