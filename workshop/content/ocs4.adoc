...
このハンズオンでは、OpenShift Container Storage Operatorを使用して、OpenShift Container Platform(OCP) 4向けの永続ストレージのソリューションである、CephとMulti-Cloud Gateway(MCG)をデプロイします。
...
OpenShift WebコンソールのStorage Dashboardでメトリクスとアラートを取得するために、`openshift-storage` namespaceには `cluster-monitoring=true` のラベルを付けることが必要です。以下のように実行して下さい。
...
また、ターミナルで次のコマンドを実行しておくと、次々とPodが作成される様子が確認できます。
...
ところで、*Openshift Web Console* のダッシュボード使用することでも、MCGステータスの概要を取得できます。左側のメニューから、*Home* -> *Dashboards* とん託し、`Object Service` タブをクリックします。
...
NOTE: 新しく追加したworker nodeにも `cluster.ocs.openshift.io/openshift-storage=` ラベルは既に付けられています。これは *machinesets* 自体にラベルの設定を行ったため、新しく作られるnodeにも自動的にラベルが付けられます。
