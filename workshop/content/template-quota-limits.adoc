## プロジェクトリクエストテンプレート、 Quotas、Limits
アプリケーション管理の基礎ラボでは、OpenShiftの基本的なビルディングブロックを扱いましたが、これらはすべて*Project*内に含まれています。

OpenShiftでは、1つのプロジェクト*内で使用できるオブジェクトの数やリソース（CPU、メモリなど）を制限しません。
...
このワークフローの中で、新たに要求されたプロジェクトを作成するためにデフォルトのプロジェクト*Template*が処理されます。
...
このオプションはデフォルトの*Template*の出力で見た `PROJECT_DISPLAYNAME` パラメータに対応します。

`new-project`ワークフローでは、ユーザーがプロジェクト要求を満たすための情報を提供します。
OpenShiftはこの要求を許可すべきかどうかを決定します。（例えば、ユーザーは*Projects*を作成する許可を持っているのか？このユーザーは*Project*を作りすぎてないか？)
もし、要求が許可できる場合は、*Template* を処理します。　

*Template* で定義されているオブジェクトを見てみると、クォータや制限についての言及がないことに気づくでしょう。
今すぐ変更しましょう。
...
新たに2つのセクションが追加されましたので、注意してください。*ResourceQuota*とLimitRange*です。
...
複数の*Project*に1つのクォータを適用したい場合は、以下を見て下さい。
...
#### Template を作成する
先ほど説明したように、*Template*はOpenShiftオブジェクトの別のタイプにすぎません。　
...
これで、`openshift-config` *Project*内に*Template*オブジェクトが作成されます。
これで、`openshift-config` プロジェクト内の *Templates* が以下のように表示されるようになりました。:
