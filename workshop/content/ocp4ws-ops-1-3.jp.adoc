...
eap70-openshift        image-registry.openshift-image-registry.svc:5000/odh-abcde/eap70-openshift        latest   About a minute ago
jboss-eap-prometheus   image-registry.openshift-image-registry.svc:5000/odh-abcde/jboss-eap-prometheus   latest   About a minute ago
----
+
...
=== 1-3-2-4. アプリケーションのアノテーション設定

JMX ExporterのServiceに対して、**annotation**をつけておきます。 +
...
[source,bash,role="execute"]
----
oc get route tcp-8080 -ojsonpath='{.spec.host}'
----
...
[source,bash,role="execute"]
----
oc get route tcp-9404 -ojsonpath='{.spec.host}'
----
